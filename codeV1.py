...
tower_model = YOLOv10('AL_tower_160924.onnx', task="detect")
...
all_steps = ["End cover fitment","End cover bolt fitment","Clamping","Interlock pin fitment","Hammering Interlock pin","Circlip fitment",
...
def record():
    ...
    if rec_vid:
        ...
        video_rec = cv2.VideoWriter(f"D:/recorded_video_al_tower/{str(uuid4())}.avi",
...
def frameInf(frame, stime):
    ...
    tower_results = tower_model.predict(
            frame, conf=0.4, iou=0.8, imgsz=640, verbose=False,device =0)
    ...
    if tower_conf.count(2)>=3:
        ...
        if 5 not in cycle_check and 4 in cycle_check and 3 in cycle_check and cls_ins.count(6)>=3:
    ...
    elif len(cycle_check)>2 and cls_ins.count(50)>=5:
        cycle_check.clear()
        tower_conf.clear()
        sequence_break_f = 0
...
def update():
    ...
    if request.method == 'GET':
        ...
        # yield "200"
        vid = cv2.VideoCapture("1724901275782.mp4") 
    while True:
        try:
            # frame = client.recv()
            ret, frame = vid.read() 
