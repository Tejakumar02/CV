...
pos_model = YOLO('models/yolov8l-pose.onnx', task='pose',)
pos_model.predict(
            "defect_scanner_logo.png", conf=0.4, iou=0.8, imgsz=640, verbose=False,device =0)
tower_model = YOLOv10('models/AL_tower_021024.onnx', task="detect")
...
all_steps = ["End cover fitment","End cover bolt fitment","Clamping","Interlock pin fitment","Hammering Interlock","Circlip fitment",
             ...
             "End cover tightening","Drain plug tightening","Cutoff valve tightening"]
cycle_check = []
cycle_check_time = []
...
def pyg(frame, details_=[], time_=[]):
    ...
    for i, val in enumerate(details):
        ...
        elif i < len(details)-1 and (0==sequence_break_f or i<sequence_break_f):
...
def frameInf(frame, stime):
    ...
    if tower_conf.count(2)>=3 or len(cycle_check)>2:
        ...
        if 14 in temp_list:
            ...
            print("drill pose: ",drillx_c,drilly_c)
            if 10 not in cycle_check:
                if 240<drillx_c<265 and 310<drilly_c<330:
                    #End cover drill
                    # cv2.putText(frame, 'End cover 1st', (20, 20),
                    #             cv2.FONT_ITALIC, 0.6, [0,0,255], 1)
                    cls_ins.append(30)
                elif 300<drillx_c<325 and 320<drilly_c<340:
                    #End cover drill
            ...
            if 12 not in cycle_check:
                if 130<drillx_c<150 and (0<drilly_c<15 or 25<drilly_c<35):
                    #Cutoff valve drill
                    # cv2.putText(frame, 'Cutoff valve 1st', (20, 20),
                    #             cv2.FONT_ITALIC, 0.6, [0,0,255], 1)
                    cls_ins.append(32)
                elif 130<drillx_c<150 and (0<drilly_c<15 or 45<drilly_c<55):
                    #Cutoff valve drill
                    # cv2.putText(frame, 'Cutoff valve 2nd', (20, 50),
                    #             cv2.FONT_ITALIC, 0.6, [0,0,255], 1)
                    cls_ins.append(32)
            if 11 not in cycle_check:
                if 420<drillx_c<438 and (145<drilly_c<157 or 113<drilly_c<125):
                    #Cutoff valve drill
                    # cv2.putText(frame, 'Drain plug', (20, 50),
                    #             cv2.FONT_ITALIC, 0.6, [0,255,0], 1)
                    cls_ins.append(31)
                elif 147<drillx_c<165 and (68<drilly_c<88 or 96<drilly_c<116):
                    #Cutoff valve drill
                    # cv2.putText(frame, 'Drain plug', (20, 50),
