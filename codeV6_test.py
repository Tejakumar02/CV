...
try:
    ...
    # older OpenCV: fall back to environment variables (may not silence all)
...
def record():
    ...
    if rec_vid:
        ...
        video_rec = cv2.VideoWriter(f"C:/Users/admin/Videos/Recorded/{str(uuid4())}.mp4",
...
def frameInf(frame):
    ...
    if tower_conf.count(2)>=3 or len(cycle_check)>2:
        ...
        if 14 in temp_list:
            ...
            cv2.putText(frame, f'Drill pos x:{drillx_c} y:{drilly_c}', (20, 150),
                        cv2.FONT_ITALIC, 1, [0,0,255], 3)
            # Tightening checks
            if 10 not in cycle_check:
                if 480<drilly_c<520 and 145<drillx_c<180:   # 169   502   152   486
                    #End cover drill
                    cv2.putText(frame, 'End cover 1st', (20, 20),
                                cv2.FONT_ITALIC, 0.6, [0,0,255], 1)
                    cls_ins.append(30)
                elif 795<drilly_c<630 and 120<drillx_c<150:    # 148   807      131     610 
                    #End cover drill
            ...
            if 12 not in cycle_check:
                if 195<drilly_c<255 and (795<drillx_c<820 or 790<drillx_c<815):   # 812   218    803  236
                    #Cutoff valve drill
                    cv2.putText(frame, 'Cutoff valve 1st', (20, 20),
                                cv2.FONT_ITALIC, 0.6, [0,0,255], 1)
                    cls_ins.append(32)
                elif 200<drilly_c<250 and (875<drillx_c<899 or 900<drillx_c<915): # 893  244   904 224
                    #Cutoff valve drill
                    cv2.putText(frame, 'Cutoff valve 2nd', (20, 50),
                                cv2.FONT_ITALIC, 0.6, [0,0,255], 1)
                    cls_ins.append(32)
            if 11 not in cycle_check:
                if 250<drilly_c<285 and (615<drillx_c<635 or 685<drillx_c<705):   #624   276       695   264
                    #Drain plug drill
                    cv2.putText(frame, 'Drain plug', (20, 50),
                                cv2.FONT_ITALIC, 0.6, [0,255,0], 1)
                    cls_ins.append(31)
                elif 890<drilly_c<915 and (580<drillx_c<600 or 515<drillx_c<535):     #  590   905        525  906
                    #Drain plug drill
                    cv2.putText(frame, 'Drain plug', (20, 50),
    ...
    frame = tower_results[0].plot(conf=False,labels=False,boxes=True)
...
def getF():
    ...
    # video_path = "rtsp://sourab:tvsm123!@10.42.0.44/stream1"
    # video_path ="rtsp://localhost:8554/mystream"
    # video_path = r"C:\projects\AL_chennai_GearBox\videos\test1.avi"
    ...
    #         # cv2.putText(frame, f"Date: {date_str} Time: {time_str}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, [0, 255, 0], 2, cv2.LINE_AA)
    #         # current_time = now.strftime("%m-%d-%Y Time- %H:%M:%S")
    #         # cv2.putText(frame, f"{current_time}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, [
    ...
    container = av.open(video_path, options={
                        "rtsp_transport": "tcp", "hwaccel": "cuda"})
    ...
    while True:
        ...
        try:
            for frame in container.decode(video=0):
                ...
                # frame = cv2.convertScaleAbs(frame, alpha=1, beta=25)
                capFrame = frame.copy()
                cv2.waitKey(1)
            else:
                print("No frame")
